		...
		for i := range tests {
			...
			It(t.name, func() {
				...
				if srcState != t.dstState {
					...
					if t.dstState == models.HostStatusDisconnected {
						severity = models.EventSeverityWarning
					}
					mockEvents.EXPECT().AddEvent(gomock.Any(), hostId.String(), severity, gomock.Any(), gomock.Any(), host.ClusterID.String())
				}
		...
		for i := range tests {
			...
			It(t.name, func() {
				...
				if *host.Status != t.dstState {
					severity := models.EventSeverityInfo
					if t.dstState == models.HostStatusError {
						severity = models.EventSeverityError
					}
					mockEvents.EXPECT().AddEvent(gomock.Any(), hostId.String(), severity,
